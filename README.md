...
2. 點擊右上角的「移動階層」按鈕
3. 在 Modal 中：
   - **選取父電表**：選擇目標父節點（或選擇「根層級」移至最上層）
   - **移動設備**：選擇要移動的設備（可多選，但必須為同層級）
4. 點擊「確認移動」
...
- ❌ **跨層級錯誤**：若嘗試選擇不同層級的設備，會顯示「所選取的設備必須在同一個層級」錯誤訊息
- ❌ 不可將節點移動到自身
- ❌ 不可將節點移動到其子孫節點下
- ❌ 不可移動到原本的位置
- ✅ 可移動到其他節點或根層級
...
- 使用 `server/db.json` 檔案存儲資料
- 資料會持久化在檔案系統中
- 所有用戶共享同一個資料檔案
- 重啟後資料保留

**生產環境（Vercel 等 Serverless 平台）**:

- 使用內存變數存儲資料（`server/utils/dataStore.ts`）
- ⚠️ **資料不會持久化**：冷啟動或重新部署後會重置為初始資料
- ⚠️ **多實例不同步**：不同的 serverless instance 之間資料不同步
- 如需生產環境持久化，建議使用雲端資料庫（如 Vercel Postgres、Supabase、MongoDB Atlas）

### 資料同步機制

**Pinia Store（前端狀態）**:

- 每個裝置/瀏覽器有獨立的 Pinia store 實例
- 選取狀態（`selectedIds`）、展開狀態（`expandedIds`）是裝置獨立的
- 不會影響其他用戶
